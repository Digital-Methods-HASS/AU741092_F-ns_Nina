...
library(leaflet)
library(tidyverse)
# uncommtidyverse# uncomment and activate whichever one worked for you to install
...
# First it is longitude, than latitude
...
# T hides the menu, and when the curser hoves over it it reappers - with that the F makes it visible at all times
...
saveWidget(student_map, "studentmap.html", selfcontained = T)
...
saveWidget(DKmap_withdata, "DKmap_withdata.html", selfcontained = T)
...
saveWidget(DKmap_withdata_withcluster, "DKmap_withdata_withclusters.html", selfcontained = T)
...
# Your brief answer: The one with no clustering is better suited for the scenario where there is fewer points with more detailed information
# with that the clustering gives an idea of where there is more points and is more suited in the case where the data shows simpler information
...
# Solution
DANmap_extradescription <- DANmap %>%  
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude, 
             popup = paste("<b> Placename: </b>", places$Placename, "<br>",
                           "<b> Description: </b>", places$Description, "<br>",
                           "<b> Type: </b>", places$Type, "<br>", 
                           "<b> Stars: </b>", places$Stars1_5, "<br>",
                           "<b> Notes: </b>", places$Notes),
             clusterOptions = markerClusterOptions(
                                                  )) 
DANmap_extradescription
...
#addMarkers(lng = places$Longitude, 
 #            lat = places$Latitude,
  #           popup = paste("<b> Placename: </b>", places$Placename, "<br>", 
   #                        "<b> Description: </b>", places$Description, "<br>", 
    #                       "<b> Type: </b>", places$Type, "<br>",
     #                      "<b> Stars: </b>", places$Stars1_5,
      #                     ))

             

saveWidget(DANmap_extradescription, "DANmap_extradescription.html")

#, selfcontained = TRUE
# "<b> Notes: </b>", ifelse(is.na(places$Notes)), "None", places$Notes,'
# , ifelse(is.na(places$Stars1_5)), "None",
#                            clusterOptions = markerClusterOptions()
