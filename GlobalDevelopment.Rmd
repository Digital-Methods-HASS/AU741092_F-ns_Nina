...
```{r}
str(gapminder) # inspecting function
unique(gapminder$year) #shows the year column
?gapminder #unfilteret is a bigger data set - with the ? it describes the dataset (or the chosen object) 
head(gapminder) #shows the tibble for the data set 
data("gapminder") #shows the data in the environment 
```
...
options(scipen = 999) # another mulighed er: scale_size_continous(labels=scales) - men er kun til en "chunk"
...
#the above subset could also use the filter 
# the aes - the x and y axis + the size of dots is the population 
#scaling the x axis to logarithmic (10) - like the videos we saw
#e - scientific notation - how do we get rid of it
...
anim + transition_states(year, 
                      transition_length = 1,
                      state_length = 1)
#splitting it up by year for each transition
...
anim2 <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop/1000000)) +
  geom_point(aes(color= continent)) +
  scale_x_log10() +
  ggtitle("World health over time") + 
  labs( x= "GDP per capita" , 
        y= "Life expectancy") +
  theme(text = element_text (size= 18)) + 
  labs(title = "World health over time",
...
ggplot(subset(gapminder, year == 2002), aes(gdpPercap, lifeExp, size = pop/1000000)) +
  geom_point(aes(color= continent)) +
  scale_x_log10() +
  ggtitle("World in 2002") + 
  labs( x= "GDP per capita" , 
        y= "Life expectancy") +
  theme(text = element_text (size= 18))

#I am born in 2001, but that data don't exist in Gapminder
...
```{r experiment with larger data set}
bigGapminder <- read_csv("data/lex.csv", na= "NA")
